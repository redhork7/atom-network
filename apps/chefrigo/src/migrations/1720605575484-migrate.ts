...
export class Migrate1720605575484 implements MigrationInterface {
    name = 'Migrate1720605575484'

    public async up(queryRunner: QueryRunner): Promise<void> {
        await queryRunner.query(`CREATE TABLE \`home\` (\`uid\` int NOT NULL AUTO_INCREMENT, \`place_name\` varchar(40) NULL COMMENT '장소명', \`created_at\` timestamp(6) NOT NULL DEFAULT CURRENT_TIMESTAMP(6), \`updated_at\` timestamp(6) NOT NULL DEFAULT CURRENT_TIMESTAMP(6) ON UPDATE CURRENT_TIMESTAMP(6), PRIMARY KEY (\`uid\`)) ENGINE=InnoDB`);
